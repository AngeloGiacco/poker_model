...
class Database:
    ...
    def __init__(self,db):
        ...
        self.cur.execute("CREATE TABLE IF NOT EXISTS DPSpace (id integer, hand text, position integer, pot integer,chip_player1 integer,chip_player2 integer,chip_player3 integer,chip_player4 integer,chip_player5 integer,chip_player6 integer, nrGames integer, previous_action text, action text, action_result integer, action_result_sum integer, action_result_avg integer)")
        self.conn.commit()
    ...
    def checkDP(self, hand, position, pot, chip_player1, chip_player2, chip_player3, chip_player4, chip_player5, chip_player6, nrGames, previous_action="Null"):
        ...
        # if there does exist a decision point then return the action taken with that decision point
        if len(rows) == 1:
